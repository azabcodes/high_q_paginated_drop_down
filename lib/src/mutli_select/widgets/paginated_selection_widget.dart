...
class PaginatedSelectionWidget<T> extends StatefulWidget {
  final List<T> items;
  ...
  final Widget Function(VoidCallback onPressed)? confirmButtonBuilder;
  ...
  const PaginatedSelectionWidget({
    super.key,
  ...
  @override
  PaginatedSelectionWidgetState<T> createState() => PaginatedSelectionWidgetState<T>();
}

class PaginatedSelectionWidgetState<T> extends State<PaginatedSelectionWidget<T>> {
  ...
  void _onScroll() {
    ...
    if (scrollController.position.pixels >= scrollController.position.maxScrollExtent - 200) {
      if (!_isFetchingMore && _hasMoreData) {
        _loadNextPage(searchBoxController.text);
      }
    }
  }
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      return _manageItemsByFilter(searchBoxController.text, isFirstLoad: true);
    });
  }

  @override
  void didUpdateWidget(covariant PaginatedSelectionWidget<T> oldWidget) {
    if (!listEquals(
        ...
        return Column(
          ...
          children: <Widget>[
                    ...
                    builder: (context, snapshot) {
                      ...
                      return RawScrollbar(
                        ...
                        child: ListView.builder(
                          ...
                          itemCount: snapshot.data!.length + (_isFetchingMore ? 1 : 0),
            ...
            if (widget.makeButtonsInRow == false) ...[
              Column(children: [_confirmButton(), _cancelButton()]),
            ],
          ],
        );
      },
  ...
  Widget _confirmButton() {
    ...
    if (widget.confirmButtonBuilder != null) {
      return widget.confirmButtonBuilder!(
        () {
          onValidate();
          if (widget.afterPopTheDialog != null) {
            widget.afterPopTheDialog!();
  ...
  Widget _cancelButton() {
    if (widget.cancelButtonProps != null) {
      if (!widget.cancelButtonProps!.isVisible) {
        return const SizedBox.shrink();
      }
      if (widget.cancelButtonProps?.builder != null) {
        return widget.cancelButtonProps!.builder!(
          () {
            if (widget.cancelButtonProps?.onTap != null) {
              widget.cancelButtonProps?.onTap!();
            }
            closePopup();
          },
        );
      ...
      return Padding(
        padding: widget.cancelButtonProps!.cancelButtonPadding ?? const EdgeInsets.symmetric(horizontal: 8),
        child: Align(
          alignment: Alignment.center,
          child: ElevatedButton(
            onPressed: () {
            ...
            style: widget.cancelButtonProps?.cancelButtonStyle,
            child: Text(
              widget.cancelButtonProps?.cancelText ?? "Cancel",
              style: widget.cancelButtonProps?.cancelTextTextStyle,
            ),
          ),
        ),
      );
    ...
    Widget defaultClearButton = Padding(
      ...
      child: Align(
        ...
        child: ElevatedButton(
          ...
          style: widget.confirmButtonStyle,
          child: Text(
            widget.clearButtonProps.clearText != null ? widget.clearButtonProps.clearText! : "clear",
            style: widget.confirmTextTextStyle,
          ),
        ),
      ),
  ...
  Future<void> _loadNextPage(String filter) async {
    ...
    try {
      final List<T> nextItems = await widget.paginatedRequest!(_page + 1, filter);
      _page++;

      _cachedItems.addAll(nextItems);

      if (nextItems.length < widget.requestItemCount) {
        _hasMoreData = false;
      }
  ...
  Future<void> _manageItemsByFilter(
    ...
    // Pagination logic has priority if provided
    if (widget.paginatedRequest != null) {
      try {
        final List<T> onlineItems = await widget.paginatedRequest!(_page, filter);
        _cachedItems.addAll(onlineItems);

        if (onlineItems.length < widget.requestItemCount) {
          _hasMoreData = false;
        }
    ...
    } else if (widget.asyncItems != null && (widget.popupProps.isFilterOnline || isFirstLoad)) {
      try {
            ...
            child: DefaultTextEditingShortcuts(
              child: Shortcuts(
                ...
                child: TextFormField(
                  ...
                  enableIMEPersonalizedLearning: searchFieldProps.enableIMEPersonalizedLearning,
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return _buildFavoriteItems(
              widget.popupProps.favoriteItemProps.favoriteItems!(
                snapshot.data!,
              ),
            );
          } else {
