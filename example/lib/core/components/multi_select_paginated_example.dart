...
class MultiSelectPaginatedExample extends StatelessWidget {
  final MultiSelectController<AnimeDataModel> controller;

  const MultiSelectPaginatedExample({Key? key, required this.controller}) : super(key: key);
        ...
        HighQDropDown<AnimeDataModel>.paginatedMultiSelect(
          ...
          paginatedRequest: (int page, String? filter) async {
            final list = await getAnimeList(
              page: page,
              queryParameters: {'page': page, "q": filter},
            );
